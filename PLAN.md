...
If `API_KEY` is unset/blank, auth is disabled entirely — every endpoint accepts requests with no `X-API-Key` header at all. This is a deliberate fail-open choice (unset config = open, not closed), for local dev convenience on a tool that's localhost-only by default anyway (§7) — not something to "fix" into fail-closed without checking first, since that's the opposite of what was explicitly asked for.
